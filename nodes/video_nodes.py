...
try:
    ...
    video_cropper_path = Path(__file__).parent.parent / "Video Cropper"
    if str(video_cropper_path) not in sys.path:
        sys.path.insert(0, str(video_cropper_path))
...
class AutoCropperNode:
    ...
    def _get_model(cls):
        ...
        if not ANIME_SEG_AVAILABLE:
            raise RuntimeError(
                "AnimeSegmentation model not available. Please ensure the Video Cropper folder is properly set up."
            )

        if cls._model is None:
            cls._device = "cuda" if torch.cuda.is_available() else "cpu"
            print(f"[AutoCropper] Loading AnimeSegmentation model on {cls._device}...")
            cls._model = AnimeSegmentation.from_pretrained("skytnt/anime-seg")
            cls._model.to(cls._device).eval()
            print("[AutoCropper] Model loaded successfully")
