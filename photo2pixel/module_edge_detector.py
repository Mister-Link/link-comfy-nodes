...
class EdgeDetectorModule(nn.Module):
    def __init__(self):
        ...
        kernel_sobel_h = (
            torch.from_numpy(kernel_sobel_h).reshape([1, 1, 3, 3]).repeat([3, 1, 1, 1])
        )
        ...
        kernel_sobel_v = (
            torch.from_numpy(kernel_sobel_v).reshape([1, 1, 3, 3]).repeat([3, 1, 1, 1])
        )
    ...
    def forward(self, rgb, param_edge_thresh, param_edge_dilate):
        ...
        edge = F.max_pool2d(
            edge,
            kernel_size=param_edge_dilate,
            stride=1,
            padding=param_edge_dilate // 2,
        )
        return edge
...
def test():
    rgb = np.array(
        Image.open("../images/example_input_mountain.jpg").convert("RGB")
    ).astype(np.float32)
